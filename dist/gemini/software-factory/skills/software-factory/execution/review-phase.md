...
- **User-facing verification** — when behavior is visible or externally observable (UI, exports, emails, CLI output, etc.): exploratory checks, screenshots, or other evidence as appropriate. For browser apps, use the Cursor browser if available, or chrome-browser-mcp if available; if no browser tool is available and browser behavior is relevant to the user's software, install or enable an appropriate browser tool before approving the review.
...
- User‑facing verification may need browser, device, or screenshot tooling when the change produces observable output. For browser apps, prefer the Cursor browser when available, use chrome-browser-mcp when that is the available browser tool, and install or enable one when browser behavior is relevant but no browser tool is available. Only that dimension should **change product code** when fixes are layout or presentation — not business logic — unless the team’s review guide says otherwise.
