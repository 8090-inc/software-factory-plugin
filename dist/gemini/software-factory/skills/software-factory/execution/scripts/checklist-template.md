...
- [ ] Exploratory pass on user-visible or external behavior — not only automated tests; for browser apps, use the Cursor browser if available, use chrome-browser-mcp if available, or install/enable a browser tool when browser behavior is relevant. Brief notes in `review-log.md` or evidence.
