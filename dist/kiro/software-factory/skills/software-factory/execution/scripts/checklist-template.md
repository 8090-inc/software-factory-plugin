...
- [ ] Exploratory pass on user-visible or external behavior — not only automated tests; for browser apps, use the Cursor browser if available, use Browser MCP / Chrome browser MCP if available, or explicitly request Browser MCP / Chrome browser MCP when browser behavior is relevant. Brief notes in `review-log.md` or evidence.
