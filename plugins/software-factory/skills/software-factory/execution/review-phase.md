...
- **User-facing verification** — when behavior is visible or externally observable (UI, exports, emails, CLI output, etc.): exploratory checks, screenshots, or other evidence as appropriate.
...
- User‑facing verification may need browser, device, or screenshot tooling when the change produces observable output; only that dimension should **change product code** when fixes are layout or presentation — not business logic — unless the team’s review guide says otherwise.
